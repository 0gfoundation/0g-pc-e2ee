...
func TestGatewayHealthz(t *testing.T) {
	gw := httptest.NewServer(newHandler(routeClient(), mustURL(t, "http://router.unused"), testOrigins(), "", "", noInFlightCap, nil, nil, discardLogger()))
	defer gw.Close()
...
func TestGatewayRouteMode(t *testing.T) {
	...
	gw := httptest.NewServer(newHandler(client, mustURL(t, router.URL), testOrigins(), "", "", noInFlightCap, nil, nil, discardLogger()))
	defer gw.Close()
...
func TestGatewayRoutesOtherRequestsToRouter(t *testing.T) {
	...
	gw := httptest.NewServer(newHandler(routeClient(), mustURL(t, router.URL), testOrigins(), "", "", noInFlightCap, nil, nil, discardLogger()))
	defer gw.Close()
...
func TestGatewayRouterPassthroughUnreachable(t *testing.T) {
	...
	gw := httptest.NewServer(newHandler(routeClient(), mustURL(t, deadURL), testOrigins(), "", "", noInFlightCap, nil, nil, discardLogger()))
	defer gw.Close()
...
func TestGatewayAccessLog(t *testing.T) {
	...
	gw := httptest.NewServer(newHandler(routeClient(), mustURL(t, upstream.URL), testOrigins(), "", "", noInFlightCap, nil, nil, logger))
	defer gw.Close()
