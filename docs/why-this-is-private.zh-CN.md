...
**你的 prompt 在离开浏览器之前就已加密，明文只在 TEE 内存中存在过。** 0G 的
router——承载并计费每一个请求的那一层——从头到尾只持有密文。这一条是无条件的：请求体
被封装给提供方 enclave 的密钥，而 router 没有那把密钥。
...
| 阶段 | 形态 | 说明 |
|---|---|---|
| 浏览器 → 网关 | **密文**（TLS） | TLS 会话在网关 TEE *内部* 终结，而不是在它前面的负载均衡器上 |
