...
**这条路径上有两个 TEE——接入网关，以及运行模型的提供方 enclave——而今天两个都由 0G
运营。** 提供方这一侧的设计意图是将来由其他方来运行；但截至目前两端都是我们，暗示相反的
事情很廉价。也正因如此，上面没有任何一句话是建立在"两个互相独立的运营方"之上的：那些保证
依赖的是**连它自己的运营方都读不进去的硬件**、**哈希由 Intel 签名的构建**，以及**你能亲自
检查这两件事**——而不是依赖我们是谁。所以我们把完整证据公开：attestation、正在运行的那份
构建的哈希、以及这份构建的来源。本文余下的部分讲的就是这件事。不去检查，你信任的就是同一
个运营方在两跳上的自述。
...
| 网关 → **0G 的 router** → 提供方 | **密文**（HPKE，封装给提供方 enclave 的密钥） | 请求没有网关到提供方的直连：它经由 router 转发，而 router 不持有能打开这个密文体的密钥 |
| 提供方 enclave 内部 | 明文，仅在内存中 | 模型就在这同一个 TEE 里运行 |
| 响应，沿同一条路径返回 | **密文** + enclave 的签名 | router 既读不到，也伪造不了 |

```mermaid
flowchart LR
    APP["你的浏览器"]

    subgraph GW["网关 TEE —— 已 attest"]
        G["TLS 在此终结"]
    end
    ...
    P -.->|"密文响应 + enclave 签名"| R
    R -.->|"仍是密文"| G
    P ---->|"响应签名，直连拉取<br/>（router 不代理这条路由）"| G
```

有一个调用确实绕开了 router：响应返回之后，网关会**直接**从提供方拉取这个响应的签名，好让
router 无法压掉或替换"这就是那个 enclave 产生的"这一证明。该调用不携带任何 prompt 内容。
