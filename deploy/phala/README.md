...
path already produced. Two knobs govern whether it can answer anything:
...
| `ZG_GATEWAY_ONCHAIN` | when off, `verdicts.onchain_signer` is `not_checked` rather than a comparison result. |
...
**It fetches nothing.** Only providers already used are reportable, records expire
after a few minutes, and no address triggers a quote fetch — so the route cannot be
